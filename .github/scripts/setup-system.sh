...
echo "Setting up this system for Karoshi development."
echo
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
	if command -v apt-get >/dev/null; then
		...
		DEBIAN_BINDGEN_DEPS="pkg-config clang" # Bindgen dependencies - it's used by a dependency of Karoshi
		...
		ARCH_BINDGEN_DEPS="clang" # Bindgen dependencies - it's used by a dependency of Karoshi
		...
		FEDORA_BINDGEN_DEPS="clang" # Bindgen dependencies - it's used by a dependency of Karoshi

		sudo dnf check-update
		sudo dnf install $FEDORA_TAURI_DEPS $FEDORA_FFMPEG_DEPS $FEDORA_BINDGEN_DEPS
		sudo dnf group install "C Development Tools and Libraries"
	else
		log_err "Your Linux distro '$(lsb_release -s -d)' is not supported by this script. We would welcome a PR or some help adding your OS to this script. https://github.com/alekseytsvetkov/karoshi/issues"
		exit 1
	fi
	...
	log_err "Your OS ($OSTYPE) is not supported by this script. We would welcome a PR or some help adding your OS to this script. https://github.com/alekseytsvetkov/karoshi/issues"
	exit 1
fi

echo "Your machine has been successfully set up for karoshi development."