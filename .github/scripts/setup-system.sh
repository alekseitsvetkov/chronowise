...
echo "Setting up this system for Chronosphere development."
echo
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
	if command -v apt-get >/dev/null; then
		...
		DEBIAN_BINDGEN_DEPS="pkg-config clang" # Bindgen dependencies - it's used by a dependency of Chronosphere
		...
		ARCH_BINDGEN_DEPS="clang" # Bindgen dependencies - it's used by a dependency of Chronosphere
		...
		FEDORA_BINDGEN_DEPS="clang" # Bindgen dependencies - it's used by a dependency of Chronosphere

		sudo dnf check-update
		sudo dnf install $FEDORA_TAURI_DEPS $FEDORA_FFMPEG_DEPS $FEDORA_BINDGEN_DEPS
		sudo dnf group install "C Development Tools and Libraries"
	else
		log_err "Your Linux distro '$(lsb_release -s -d)' is not supported by this script. We would welcome a PR or some help adding your OS to this script. https://github.com/alekseytsvetkov/chronosphere/issues"
		exit 1
	fi
	...
	log_err "Your OS ($OSTYPE) is not supported by this script. We would welcome a PR or some help adding your OS to this script. https://github.com/alekseytsvetkov/chronosphere/issues"
	exit 1
fi

echo "Your machine has been successfully set up for chronosphere development."